...
import com.devsuperior.uri2737.dto.LawyerAllDto;
...
public class Uri2737Application implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
        ...
        List<LawyerAllProjection> listAll = repository.search();
        List<LawyerAllDto> resultAll = listAll.stream()
                .map(LawyerAllDto::new).collect(Collectors.toList());

        System.out.println("Lista de toda tabela");
        for (LawyerAllDto objForAll : resultAll) {
            System.out.println(objForAll);
        }
